            ...
            httpSrv.alist(data).then(
                res => {
                    if(res.data.logout) {
                        ...
                        alert(res.data.message);
                    } else if (Array.isArray(res.data)) { // set the user data as the staff data
                        setStaffs(res.data);
                    }
                },
                rej => {
                ...
                <div className="col">
                    <div className="table-responsive">
                        <table className="table table-secondary">
                            ...
                            <tbody>
                                {staffs.length != 0 ? (staffs.map((staff, idx) => (<StaffRow key={idx} staff={staff} approve={appr} />))) : (<tr><td colSpan="6">No waiting staff</td></tr>)}
                            </tbody>
                        </table>
                    </div>
                </div>
